...
public class InRoom2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_in_room2);
        bind();
    }
    ...
    protected void onStart() {
        ...
        if(mUser == null){
            startActivity(new Intent(InRoom2Activity.this,MainActivity.class));
            finish();
        }
        pageManage();
    }
    ...
    private final View.OnClickListener  onClickListener = new View.OnClickListener(){
        ...
        public void onClick(View v) {
            switch (v.getId()){
                ...
                case R.id.cured_room2:
                    curedPatient();
                    Toast.makeText(InRoom2Activity.this, "Cured This Patient", Toast.LENGTH_SHORT).show();
                ...
                case R.id.choose_room2:
                    startActivity(new Intent(getApplicationContext(), Room2ChooseActivity.class));
                    break;

                case R.id.refresh_room2:
                    mReadRoomQueue();
    ...
    private void bind(){
        doc1 = findViewById(R.id.doctor_room2);
        queue1 = findViewById(R.id.queue_room2);
        name1 = findViewById(R.id.name_room2);
        gen1 = findViewById(R.id.gend_symb2);
        birth1 = findViewById(R.id.birth_room2);
        blood1 = findViewById(R.id.blood_room2);
        weigh1 = findViewById(R.id.weight_room2);
        height1 = findViewById(R.id.height_room2);
        cau1 = findViewById(R.id.caution_room2);
        note1 = findViewById(R.id.note_room2);
        pstate1 = findViewById(R.id.pstate_room2);
        phone1 = findViewById(R.id.phone_room2);

        exitB = findViewById(R.id.exit_room2);
        enterB = findViewById(R.id.enter_room2);
        curedB = findViewById(R.id.cured_room2);
        findNextB = findViewById(R.id.findNext_room2);
        chooseB = findViewById(R.id.choose_room2);
        refB = findViewById(R.id.refresh_room2);
    ...
    private void pageManage(){
        ...
        mRoom1 = mDatabase.getReference("Clinic").child("rooms").child("2");
            ...
            for (int i = 0; i < patientKeys.size(); i++) {
                if (patientKeys.get(i).equals(patientID.get(0))) {
                    ...
                    if (thisStatus.get(roomQ.get(0)-1).toString().equals("waiting") && isInRoomSet == false) {
                        manualReadAllQueue();
                        mAllQueue.child(String.valueOf(roomQ.get(0))).child("status").setValue("inRoom2");
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot node : dataSnapshot.getChildren()){
                    if(node.child("status").exists()){
                        if(node.child("status").getValue().toString().equals("inRoom2")){
    ...
    private void curedPatient(){
        ...
        if(thisStatus.get(roomQ.get(0)-1).toString().equals("inRoom2")){
            mReadPatientID();
        ...
        mAllQueue.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if(waitingList.isEmpty()){
                    Toast.makeText(InRoom2Activity.this, "Don't have any Waiting Patient now", Toast.LENGTH_SHORT).show();
                }else{
                    ...
                    if(oldPatient > 0) {
                        if (thisStatus.get(oldPatient-1).toString().equals("inRoom2")) {
    ...
    private void setOfficerRoom(){
        mOfficers.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot node : dataSnapshot.getChildren()){
                    if(node.child("status").exists()){
                        if(node.child("status").equals("Room2")){
                            mOfficers.child(node.getKey()).child("status").setValue("idle");
                        }
                    }
                    if(node.getKey().equals(mUser.getUid())){
                        mOfficers.child(node.getKey()).child("status").setValue("Room2");
                    }
                }
            }
