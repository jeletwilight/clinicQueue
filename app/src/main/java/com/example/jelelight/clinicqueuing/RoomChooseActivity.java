...
public class RoomChooseActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mRecyclerView = findViewById(R.id.recycler_queues_c1);
        bind();
    ...
    private void bind(){
        ...
        queue_id_edit = findViewById(R.id.choose_id_room1);
        choose_btn = findViewById(R.id.choose_btn_room1);
        back_btn = findViewById(R.id.back_btn_c1);
        ...
        choose_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(queue_id_edit.getText().toString() != "" && queueList.contains(queue_id_edit.getText().toString())) {
                    mSetWaitQueues();
                    mRefRoom.child("1").child("queue").setValue(Integer.valueOf(queue_id_edit.getText().toString()));
                    for(int i = 0; i<idList.size(); i++){
                        if(idList.get(i).toString() == queue_id_edit.getText().toString()){
                            mRefRoom.child("1").child("patient").setValue(nameList.get(i).toString());
                        }
                    }
                    setInRoom();
                    Toast.makeText(RoomChooseActivity.this, "Patient Choosed", Toast.LENGTH_SHORT).show();

                }else{
                    Toast.makeText(RoomChooseActivity.this, "Please Enter Queue ID", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void mSetWaitQueues(){
        mRefQueues.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot keyNode : dataSnapshot.getChildren()){
                    if(keyNode.child("status").getValue().toString().equals("inRoom1")){
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot keyNode : dataSnapshot.getChildren()){
                   if(keyNode.getKey().toString().equals(queue_id_edit.getText().toString())){
                       mRefQueues.child(keyNode.getKey()).child("status").setValue("inRoom1");
                   }
                }
            }
