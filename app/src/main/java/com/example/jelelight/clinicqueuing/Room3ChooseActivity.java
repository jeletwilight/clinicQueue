...
public class Room3ChooseActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_room3_choose);
        mRecyclerView = findViewById(R.id.recycler_queues_c3);
        bind();
    ...
    private void bind(){
        ...
        queue_id_edit = findViewById(R.id.choose_id_room3);
        choose_btn = findViewById(R.id.choose_btn_room3);
        back_btn = findViewById(R.id.back_btn_c3);
        ...
        choose_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(queue_id_edit.getText().toString() != "" && queueList.contains(queue_id_edit.getText().toString())) {
                    mSetWaitQueues();
                    mRefRoom.child("3").child("queue").setValue(Integer.valueOf(queue_id_edit.getText().toString()));
                    for(int i = 0; i<idList.size(); i++){
                        if(idList.get(i).toString() == queue_id_edit.getText().toString()){
                            mRefRoom.child("3").child("patient").setValue(nameList.get(i).toString());
                        }
                    }
                    setInRoom();
                    Toast.makeText(Room3ChooseActivity.this, "Patient Choosed", Toast.LENGTH_SHORT).show();

                }else{
                    Toast.makeText(Room3ChooseActivity.this, "Please Enter Queue ID", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void mSetWaitQueues(){
        mRefQueues.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot keyNode : dataSnapshot.getChildren()){
                    if(keyNode.child("status").getValue().toString().equals("inRoom3")){
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot keyNode : dataSnapshot.getChildren()){
                    if(keyNode.getKey().toString().equals(queue_id_edit.getText().toString())){
                        mRefQueues.child(keyNode.getKey()).child("status").setValue("inRoom3");
                    }
                }
            }
