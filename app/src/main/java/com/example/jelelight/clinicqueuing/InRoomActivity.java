...
public class InRoomActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        if(mUser == null){
            startActivity(new Intent(InRoomActivity.this,MainActivity.class));
            finish();
        }
        pageManage();
    }
    ...
    private final View.OnClickListener  onClickListener = new View.OnClickListener(){
        ...
        public void onClick(View v) {
            switch (v.getId()){
                ...
                case R.id.cured_room1:
                    curedPatient();
                    Toast.makeText(InRoomActivity.this, "Cured This Patient", Toast.LENGTH_SHORT).show();
                ...
                case R.id.choose_room1:
                    startActivity(new Intent(getApplicationContext(),RoomChooseActivity.class));
                    break;

                case R.id.refresh_room1:
                    mReadRoomQueue();
    ...
    private void bind(){
        doc1 = findViewById(R.id.doctor_room1);
        queue1 = findViewById(R.id.queue_room1);
        name1 = findViewById(R.id.name_room1);
        gen1 = findViewById(R.id.gend_symb);
        birth1 = findViewById(R.id.birth_room1);
        blood1 = findViewById(R.id.blood_room1);
        weigh1 = findViewById(R.id.weight_room1);
        height1 = findViewById(R.id.height_room1);
        cau1 = findViewById(R.id.caution_room1);
        note1 = findViewById(R.id.note_room1);
        pstate1 = findViewById(R.id.pstate_room1);
        phone1 = findViewById(R.id.phone_room1);

        exitB = findViewById(R.id.exit_room1);
        enterB = findViewById(R.id.enter_room1);
        curedB = findViewById(R.id.cured_room1);
        findNextB = findViewById(R.id.findNext_room1);
        chooseB = findViewById(R.id.choose_room1);
        refB = findViewById(R.id.refresh_room1);
    ...
    private void pageManage(){
        ...
        mRoom1 = mDatabase.getReference("Clinic").child("rooms").child("1");
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot node : dataSnapshot.getChildren()){
                    if(node.child("status").exists()){
                       if(node.child("status").getValue().toString().equals("inRoom1")){
    ...
    private void curedPatient(){
        ...
        if(thisStatus.get(roomQ.get(0)-1).toString().equals("inRoom1")){
            mReadPatientID();
        ...
        mAllQueue.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if(waitingList.isEmpty()){
                    Toast.makeText(InRoomActivity.this, "Don't have any Waiting Patient now", Toast.LENGTH_SHORT).show();
                }else{
                    mRoom1.child("queue").setValue(Integer.valueOf(waitingList.get(0)));
                    mReadRoomQueue(); // to waiting 1
                    //mAllQueue.child(String.valueOf(roomQ.get(0))).child("status").setValue("inRoom1");
                    if(oldPatient > 0) {
                        if (thisStatus.get(oldPatient-1).toString().equals("inRoom1")) {
    ...
    private void setOfficerRoom(){
        mOfficers.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot node : dataSnapshot.getChildren()){
                    if(node.child("status").exists()){
                        if(node.child("status").equals("Room1")){
                            mOfficers.child(node.getKey()).child("status").setValue("idle");
                        }
                    }
                    if(node.getKey().equals(mUser.getUid())){
                        mOfficers.child(node.getKey()).child("status").setValue("Room1");
                    }
                }
            }
